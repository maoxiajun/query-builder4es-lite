...
import com.fordeal.search.base.QueryException;
...
public class Range extends BaseAggs<Range> {
    ...
    public Range interval(Comparable from, Comparable to) {
        if (null != from && null != to) {
            if (from.compareTo(to) >= 0) {
                throw new QueryException("from should not be greater than to");
            }
        }
    ...
    public Object value() {
        ...
        if (ranges.isEmpty()) {
            throw new QueryException("range aggregation must contains 1 range sub condition at least");
        }
    ...
    private void addRange(Comparable from, Comparable to) {
        if (null == from && null == to) {
            throw new QueryException("from & to should not be null both");
        }
        Map<String, Object> range = Maps2.of();
        if (null != to) {
            range.put("to", to);
        }
        if (null != from) {
